...
function PassGen() {
  ...
  if(!localStorage.getItem("passArr")){
    ...
    passArr = JSON.parse(localStorage.getItem("passArr"))
  }
      ...
      <div className="mainContainer w-[40vw]">
          ...
          <div className="passwordLengthContainer flex flex-col">
            ...
            <input
              ...
              onChange={() => {
                ...
                let passValue = getPasswordValue.value;
                setPasswordLength(passValue);
              }}
            />
          </div>
        </div>
        ...
        <div className="buttonField flex gap-4 mt-4">
          <button
            ...
            onClick={() => {
              const getSetPassword = generatePassword(passwordLength)
              setPassword(getSetPassword);
            }}
          >
            Generate
          </button>
          <button className="savePassword px-6 py-2 bg-red-400 rounded-xl" onClick={()=>{
            const usernameValue = document.getElementById('username')
            const userVal = usernameValue.value
            const applicationName = document.getElementById('platformName')
            const applicationVal = applicationName.value
            const getSetPassword = password
            savePassword(passArr, applicationVal, userVal, getSetPassword);
          }}>
            Save
          </button>
        </div>
